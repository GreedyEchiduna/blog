...
date: '2023-03-24'
lastmod: '2023-03-24'
tags: ['rust', 'FP', 'Universe_Function']
draft: false
summary: '本文将通过例子讨论Rust中的函数参数的使用方法和UniverseFunction机制'
authors: ['default']
layout: PostLayout
---
...
这个是[迭代器中的`max`](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.max)中的内容, 在这里, 标准库写道: 因为浮点数并没有实现`Ord Trait`, 因此对于一个由浮点数组成的集合转化而来的迭代器, 我们没有办法直接调用`max`方法获取迭代器中元素的最大值, 因为这个方法依赖于`Ord Trait`, 因此, 就有了如上的处理方式.
